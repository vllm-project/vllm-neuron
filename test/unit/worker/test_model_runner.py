...
class TestModelRunner:
    ...
    def model_runner(self, vllm_config, mock_model):
        runner = NeuronxDistributedModelRunner(
    ...
    def test_update_states(self, model_runner, mock_scheduler_output):
        ...
        print(f"mock_cached_reqs.new_block_ids: {mock_cached_reqs.new_block_ids}")
        print(f"mock_cached_reqs[0]: {mock_cached_reqs[0]}")
        
        # Debug prints and assertions
        test_block_ids = mock_req_state.block_ids[0]
        test_new_ids = mock_scheduler_output.scheduled_cached_reqs.new_block_ids[0]
        print(f"test_block_ids: {test_block_ids}, type: {type(test_block_ids)}")
    ...
    def test_process_new_request(self, model_runner):
        ...
        model_runner._process_new_request_for_continuous_batching(request_data, data)
    ...
    def test_get_kv_cache_spec(self, model_runner):
        ...
        assert spec["layer"].num_kv_heads == model_runner.model.num_key_value_heads
