#!/bin/bash
# GPT-OSS 20B with DFlash on a single Trn2 chip (trn2.3xlarge) using TP4 and BF16.
#
# A Trn2 chip at the default logical-NeuronCore config (LNC=2) exposes four
# logical cores, so TP4 is the only tensor-parallel degree available on a
# 3xlarge. LNC=1 is not an alternative: the nkilib decode kernels this plugin
# calls assume LNC=2. Use run_tp8.sh on a 48xlarge.
#
...
echo "  One Trn2 chip, BF16, TP4, 7 speculative tokens, ${NUM_BLOCKS} KV blocks"
